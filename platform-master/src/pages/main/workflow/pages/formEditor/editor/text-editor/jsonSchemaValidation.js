 ...
 *  Schemas to register
 */
export const addSchema = (editor, schemas) => {
 ...
 */
export const configureJsonSchemaValidation = (editor, modelUri) => {
 ...
 */
export const configureRuleSchemaValidation = (editor, modelUri) => {
  /** Note that the Monaco Editor only supports JSON Schema Draft 7 itself,
   * so if we also want to support a later standard we still have to formalize
   * it in JSON Schema Draft 7*/
  addSchema(editor, [
...
export const getMonacoModelForUri = (modelUri, initialValue) => {
